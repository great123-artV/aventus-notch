...
const fetchCryptoPrices = async (): Promise<CryptoPrice[]> => {
  ...
  return data.map((coin: any) => ({
    ...
    marketCap: (coin.market_cap ?? 0) > 1e12 ? `${((coin.market_cap ?? 0) / 1e12).toFixed(2)}T` : `${((coin.market_cap ?? 0) / 1e9).toFixed(1)}B`,
    volume: (coin.total_volume ?? 0) > 1e9 ? `${((coin.total_volume ?? 0) / 1e9).toFixed(1)}B` : `${((coin.total_volume ?? 0) / 1e6).toFixed(0)}M`,
